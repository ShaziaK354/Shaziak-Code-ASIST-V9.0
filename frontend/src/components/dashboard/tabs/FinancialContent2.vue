  ...
  setup(props) {
    ...
    const groupedFinancialRecords = computed(() => {
      ...
      records.forEach(record => {
        ...
        groups[rsn].totalOaRec += parseFloat(record.adjusted_net_rsn || record.oa_rec_amt || record.dir_rsrv_amt || 0);
      ...
      Object.values(groups).forEach(group => {
        ...
        group.totalAvailable = group.totalOaRec - group.totalNetCommit - group.totalGrossObl;
      });
    ...
    const calculateRecordBalances = (record) => {
      const oaRec = parseFloat(record.adjusted_net_rsn || record.oa_rec_amt || record.dir_rsrv_amt || 0);
      ...
      return {
        ...
        totalAvailable: oaRec - netCommit - netObl
      };
    };
