<template>
  <aside class="chat-interface-panel" :class="{ 'collapsed': isCollapsed }">
    <!-- Toggle Button -->
    <button class="panel-toggle-btn" @click="togglePanel" :title="isCollapsed ? 'Expand AI Assistant' : 'Collapse AI Assistant'">
      <i class="fas" :class="isCollapsed ? 'fa-chevron-left' : 'fa-chevron-right'"></i>
    </button>
      ...
      <div class="chat-messages" ref="chatMessagesContainerRef">
        ...
        >
          ...
          <div class="message-content">
            ...
            <!-- Removed: Sent with Context display -->
          ...
          try {
            ...
            switch(jsonData.type) {
              ...
              case 'answer_complete':
                if (jsonData.answer) {
                  console.log('[Streaming] Answer complete - updating with linked version');
                  chatStore.currentSessionMessages[aiMessageIndex].text = jsonData.answer;
                  scrollToBottom();
                }
                break;
              case 'complete':
...
.chat-interface-panel.collapsed {
  flex: 0 0 50px;
  min-width: 50px;
  padding: 0;
}

.chat-interface-panel.collapsed .panel-content {
  opacity: 0;
  visibility: hidden;
  pointer-events: none;
}
...
.chat-interface-panel.collapsed .panel-toggle-btn {
  left: -15px;
  border-radius: 8px 0 0 8px;
}
...
.chat-input textarea.ai-chat-input {
  flex: 1;
  border: none;
  outline: none;
